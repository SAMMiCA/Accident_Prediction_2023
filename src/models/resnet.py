...
def freeze_bn(m, name):
    for attr_str in dir(m):
        ...
        if type(target_attr) == torch.nn.BatchNorm3d:
            frozen_bn = FrozenBN(
                target_attr.num_features, target_attr.momentum, target_attr.eps
            )
            frozen_bn.set_params(
                target_attr.weight.data,
...
class I3Res50(nn.Module):
    ...
    def forward_multi(self, x):
        ...
        for clip_idx in range(x.shape[1]):  # B, 10, 3, 3, 32, 224, 224
            ...
            spatial_crops = torch.stack(spatial_crops, 1)  # (B, 3, 2048)
            # print(spatial_crops.size())
            clip_preds.append(spatial_crops)
        # clip_preds = torch.stack(clip_preds, 1).mean(1)  # (B, 400)
        clip_preds = torch.stack(clip_preds, 1)  # changed
...
if __name__ == "__main__":
    net = i3_res50_nl(400)
    # inp = {'frames': torch.rand(4, 3, 700, 224, 224)}
    # inp = {'frames': torch.rand(1, 3, 32, 224, 224)}
    inp = {"frames": torch.rand(1, 2, 4, 3, 8, 224, 224)}
    # inp = {"frames": torch.rand(1, 25, 10, 3, 2, 224, 224)}
    pred, losses = net(inp)
